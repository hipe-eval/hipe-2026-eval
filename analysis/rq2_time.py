...
Reads analysis.d/tables/pair_level_features.parquet (read-only). Writes:
  ...
  analysis.d/figures/rq2_time.pdf
"""
...
from plotting_common import TASK_COLORS, new_figure, save_figure, style_axis
...
def build_time_table(df: pd.DataFrame, bin_edges: list[int], include_isat: bool) -> pd.DataFrame:
    df = df.copy()
    df["year_bin"] = pd.cut(df["year"], bins=bin_edges, right=False, include_lowest=True)
...
def main() -> int:
    ...
    for ax, table, title, include_isat in [
        (axes[0], table_a, "Domain A (impresso)", True),
        (axes[1], table_b, "Domain B (surprise)", False),
    ]:
        x = list(range(len(table)))
        labels = list(table["year_bin"].astype(str))
        ...
        ticks = [f"{label}\n(n={n})" for label, n in zip(labels, table["n_pairs"])]
        ax.set_xticks(x)
        ax.set_xticklabels(ticks, rotation=45, ha="right", fontsize=5)
        style_axis(ax)
