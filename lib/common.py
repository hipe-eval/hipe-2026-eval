...
SUBMISSION_RE = re.compile(r"^(team\d+)_(.+)_run([1-3])\.jsonl$")
INFO_RE = re.compile(r"^(team\d+)_(.+)_run([1-3])-info\.json$")
...
def parse_submission_filename(path_or_name: Path | str) -> SubmissionName:
    ...
    if not match:
        raise ValueError(
            f"Invalid submission filename '{name}'. Expected teamN_<reference-stem>_runX.jsonl with X in 1..3."
        )
